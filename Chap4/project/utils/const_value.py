...
"""
    ...
    参数说明：
        ...
        lang：
            lang=zh_cn, 中文
            """
KEY_OWM = '7dd907ccf0bf04b078d04403cfea138c'  # 原子申请的 API key，10分钟调用一次
...
def initialApiParams(location):
    APIParamsDict = {}

    params_XZ = {"key":KEY_XZ,"unit":UNIT_XZ,"language":LANGUAGE_XZ,"location":location}
    XZAPI = {"API":API_XZ, "params":params_XZ, "text":"XZ_results_now_text", "strTemp":"XZ_results_now_temperature", "time":"XZ_results_last_update"}
    APIParamsDict["XZ"] = XZAPI

    # params_OWM = {"appid":KEY_OWM,"units":UNIT_OWM,"q":location}
    # OWMAPI = {"API":API_OWM,"params":params_OWM,"text":"OWM_weather_main", "strTemp":"OWM_main_temp", "time":""}
    # APIParamsDict["OWM"] = OWMAPI
    #
    # ysl = "select * from weather.forecast where woeid in (select woeid from geo.places(1) where text='" + location + "')"
    # params_Yahoo = {"format":"json","q":ysl}
    # YahooAPI = {"API":API_Yahoo,"params":params_Yahoo,"text":"Yahoo_query_results_channel_item_condition_text", "strTemp":"Yahoo_query_results_channel_item_condition_temp", "time":"Yahoo_query_results_channel_item_condition_date"}
    # APIParamsDict["Yahoo"] = YahooAPI

    return APIParamsDict
